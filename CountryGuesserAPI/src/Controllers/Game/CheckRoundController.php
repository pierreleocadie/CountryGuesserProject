<?php 

namespace CountryGuesser\Controllers;

require_once __DIR__ . "/../../../vendor/autoload.php";

use CountryGuesser\Lib\RequestDataCompliance;
use CountryGuesser\Models\Game\CheckRound;

/*

    Steps to check if a round exist:
    - Recover data from request
    - Check if the data is in JSON format -> If not, we send an error
    - Secure the data
    - Check if the data correspond to what is expected
        - If YES :
            - We check if the round exist -> return a boolean
        - If NO :
            - We send an error

*/

$userData = file_get_contents("php://input");
if($userData && RequestDataCompliance::checkIfItsJSON($userData))
{
    $userData = json_decode($userData, true);
    if(RequestDataCompliance::CheckRoundDataCompliance($userData)){
        $game = new CheckRound();
        echo $game->checkRound($userData["game_id"], $userData["round_id"]);
    }else{
        echo json_encode(["error" => "Invalid data", "details" => "game_id -> int, round_id -> int"]);
    }
}else{
    echo json_encode(["error" => "No data or invalid format", "details" => "Must be JSON format"]);
}